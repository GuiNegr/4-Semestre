...
public class JogoPTBR {
    static protected Pokemon player_one;
    static protected Pokemon player_two;

    private static void gameLogo() {
        System.out.println("---------------------------------------------------");
        System.out.println("---------BEM VINDO PARA O  POKEMON LEGENDS JAVA-----------");
        System.out.println("-------PRIMEIRA ADO DE ARQUITETURA DE SOFTWARE------");
        System.out.println("---------------------------------------------------");
    }

    protected static void gameTurn(Pokemon Player_one_OutClass, Pokemon Player_two_OutClass) {
        gameLogo();
        player_one = MenuPTBR.MenuChooserCharacter(Player_one_OutClass);
        player_two = MenuPTBR.MenuChooserCharacter(Player_two_OutClass);
        int[] choses = new int[2];

        while(player_one.vida() > 0 || player_two.vida() > 0){
            System.out.println("PLAYER UM: "+player_one.getClass().getCanonicalName());
            System.out.println("| - - - - - - - - - - - - |");
            System.out.println("|"+player_one.vida()+"| HP - - - - - - - - |");
            System.out.println("| - - - - - - - - - - - - |");
            choses[0] = MenuPTBR.turn();
        ...
        if (player_one.vida() > 0 && player_two.vida() < 0){
            System.out.println("VITORIA "+player_one.getClass().getCanonicalName());
        }else if(player_two.vida() > 0 && player_one.vida() < 0){
            System.out.println("VITORIA "+player_two.getClass().getCanonicalName());
        }
        System.out.println("FEITO POR  GUILHERME NEGREIROS");
    }
}
